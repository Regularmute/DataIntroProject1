...
class Strategy(Enum):
    ...
    ONE_MONTH_RANDOM_TAIL = 3           # Try to predict next n days given one month training 
    THREE_MONTHS_RANDOM_TAIL = 4        # Same as above with 3 month training
    ONE_YEAR_FULL = 5                   # Use whole year for training and predict random days
    ONE_MONTH_FULL = 6                  # Train model with one month, predict every day of the year
...
def run_strategy(strat:Strategy, plot_type:PlotType, days=0, print_=True):
    ...
    if not isinstance(strat, Strategy):
        raise ValueError(f"Invalid strategy type: {type(strat)}. Expected a Strategy enum.")
    ...
    if strat == Strategy.ONE_MONTH_RANDOM:
        for month in range(1,2):
            ...
            for i in range(0, days):
                day = DFH.get_day()
                # Get prediction here
                actual, pred = func.predict_co2_for_day(model, day, day.date[0], print_=False)
                comparison = func.compare_predictions(actual, pred, False)
                print("Month",month," trained model ----> predict:",day.date[0],"===============================================")
        ...
        if print_:
            print("====================================================================================")
            print(f"Predicted {days} random days based on model trained with data from 90 previous days")
            print("AVG results:")
        ...
        if print_:    
            print("====================================================================================")
            print(f"Predicted {days} random days based on model trained with data from 30 previous days")
            print("AVG results:")
            print(result_df.mean())
        ...
        if plot_type == PlotType.METRICS:
            fig, axs = plt.subplots(1,4, figsize=(20,20)) # Four figures for each model
        else:
            fig, axs = plt.subplots(1,1, figsize=(20,20)) # One figure for each model
        ...
        for n in range(0,365):
            ...
            if plot_type == PlotType.METRICS:
                ...
                new_row = pd.DataFrame([{'Actual':actual.mean(), 'Predicted':pred.mean()}])
        ...
        first_day = datetime(2023, 1, 1)
        if plot_type == PlotType.METRICS:
            fig, axs = plt.subplots(12,4, figsize=(20,20)) # Four figures for each model
        else:
            fig, axs = plt.subplots(2, 6, figsize=(20,20)) # One figure for each model
        ...
        first_day = datetime(2023, 1, 1)
        if plot_type == PlotType.METRICS:
            fig, axs = plt.subplots(10, 4, figsize=(20,20)) # Four figures for each model
        else:
            fig, axs = plt.subplots(2, 6, figsize=(80,20)) # One figure for each model
...
def plot_metrics(df, month, axs, red_line_limit):
    ...
    axs[month, 1].set_title(f"Month {month+1} average RMSE: {df['rmse'].mean()}")
    axs[month, 1].set_xticks([])

    axs[month, 2].plot(df.index, df["mape"])
    axs[month, 2].plot(red_line_limit, [0, 0], color='red', lw=2)
    axs[month, 2].set_title(f"Month {month+1} average MAPE: {df['mape'].mean()}")
...
def plot_predictions(df, month, axs, red_line_limit):
    ...
    axs[int(month/6), int(month%6)].plot(df.index, df["Actual"], color='blue')
    axs[int(month/6), int(month%6)].plot(df.index, df["Predicted"], color='orange')
    axs[int(month/6), int(month%6)].plot(red_line_limit, [0, 0], color='red', lw=2)
    axs[int(month/6), int(month%6)].set_title(f'{month+1} Prediction (orange)')

if __name__ == '__main__':
    path = 'data/data_2023.csv'
    DFH = dataframe_handler.DFHandler(path)
    DFH.initialize()
    DFH.set_renormalize_limit(False)    # Don't use renormalizing on time limited dataframe
