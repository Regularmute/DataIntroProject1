...
periodic = [
    ...
    # 'electricity_cost',
    # 'year',
    'month',  # tämä oli mukana, debug poisto
    # 'day',
    'day_of_week',
]
...
class DFHandler:
    ...
    def __init__(self, path=None, df=None):
        ...
        if df is not None:
            ...
            raise ValueError(
                "Dataframe handler should be initialized with dataframe or path instance to csv-source")
    ...
    def limit(self, s_year=None, s_month=None, e_year=None, e_month=None, s_day=1, e_day=None, start=None, end=None):
        ...
        if start and end:
            ...
            print(
                f"CREATING NEW RANGED DF {s_year}:{s_month}:{s_day}-{e_year}:{e_month}:{e_day}")
    ...
    def avg_temperatures(self, df=None):
        ...
        if df is None:
            ...
            DF = df
        DF['temp'] = DF[[col for col in DF.columns if 'temperature' in col]].mean(
            axis=1)
        if df is None:
            self.initialized_df = DF
            return self
    ...
    def drop_columns(self, df=None):
        ...
        if df is None:
            DF = self.initialized_df
            drop_columns = [
                column for column in DF.columns if column in dropped and column in DF.columns]
    ...
    def initialize(self, df=None, avg_temp=True, create_dates=True, to_periodic=True, normalize=True, drop_columns=True, decay=False):
        ...
        """
        print(
            f"RUN INITIALIZATION on {('class instance' if df is None else 'given dataframe')}")
        if df is None:
            # Running initialize always resets first to original dataframe
            self.reset()
        if avg_temp:
            # Average temperatures
            self.avg_temperatures(df)
        if create_dates:
            # Turn date-column to date-objects
            self.create_dates(df)
        if to_periodic:
