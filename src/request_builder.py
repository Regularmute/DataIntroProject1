...
def get_fingrid_url(datasetId, s_year, s_month, s_day, s_hour, s_min, e_year, e_month, e_day, e_hour, e_min):
    ...
    return f'https://data.fingrid.fi/api/datasets/{datasetId}/data?startTime={startTime}&endTime={endTime}&format=json&pageSize=750'
...
def get_dataframe_by_url(url):
    ...
    if r.status_code == 200:
        ...
        time = df['startTime'].dt.strftime('%Y-%m-%d-%H:%M').str.split(r'[:\-\.TZ]', expand=True).iloc[:,0:5]
